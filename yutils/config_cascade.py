...
def append_to_cascade(name, previous=None):
...
class CascadingConfig(dict):
    ...
    def get(self, name, default=None, _prev=None):
        ...
        if name in self:
            ...
            _prev = append_to_cascade(name, _prev)
    ...
    def cascade_list(self, name, _prev=None):
        ...
        path = []
        while True:
            path = append_to_cascade(name, path)
            try:
...
def test_get_cascade_multi():
    ...
    config = CascadingConfig({'size': 0}, cascade_map)
    assert config.get('arrow.size') == 0
